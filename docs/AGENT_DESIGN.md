...
- `SqliteSaver` at `data/checkpoints.db` (added to `.gitignore`).
- Thread ID = session UUID, generated by the frontend on first message and stored in a browser cookie (`bc_wine_session_id`).
